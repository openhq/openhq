...
RSpec.describe "Team Invites API", type: :api do
  ...
  describe "POST /api/v1/team_invites" do
    ...
    it "invites a new user to the team" do
      ...
      expect(response_json[:team_invites].count).to eq(1)
      expect(response_json[:team_invites].first[:team_id]).to eq(user.teams.first.id)
      expect(response_json[:team_invites].first[:role]).to eq("user")
      expect(response_json[:team_invites].first[:status]).to eq("invited")
      expect(response_json[:team_invites].first[:invitation_code]).not_to be_empty
      expect(response_json[:team_invites].first[:invited_at]).not_to be_empty
      expect(response_json[:team_invites].first[:invited_by]).to eq(user.id)
    end
    ...
    it "adds projects to the invited user" do
      ...
      expect(response_json[:projects].first[:name]).to eq("Test Project")
    end
    ...
    it "does not add projects the current user is not part of" do
      ...
      expect(response_json[:projects].count).to eq(1)
      expect(response_json[:projects].first[:name]).to eq("Test Project")
    end

    it "will not invite a user if they are already on the team" do
      post "/api/v1/team_invites", { user: { email: user.id } }, api_token_header(user)
      expect(response_json[:team_invites]).to be_nil
    end
    ...
    it "will add projects to an already invited user" do
      ...
      post "/api/v1/team_invites", { user: { email: "invite@openhq.io", project_ids: [project.id] } }, api_token_header(user)
      expect(response_json[:projects]).not_to be_empty
    end
  end
  ...
  describe "PUT /api/v1/team_invites/:id" do
    ...
    it "activates an invited user" do
      invite_code = response_json[:team_invites].first[:invitation_code]
      ...
      expect(last_response.status).to eq(200)
      expect(response_json[:team_invites]).to be_empty
      expect(response_json[:team_users].count).to eq(1)
