...
RSpec.feature "Stories", type: :feature do
  ...
  given!(:sarah) { create(:user, username: "sarah", email: "sarah@example.org", password: "hunter212", password_confirmation: "hunter212") }
  background do
    ui_design = bob.projects.create!(name: "UI Design", owner: bob)
    engineering = sarah.projects.create!(name: "Engineering", owner: sarah)

    bob.projects << engineering

    @story = engineering.stories.create!(name: "Speed tunnel testing", owner: bob)

    @story.comments.create!(content: "Hey @bob", owner: sarah)
  end
